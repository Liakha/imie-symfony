...
use Acme\AppBundle\Entity\Link;
use Acme\AppBundle\Form\LinkType;
...
class LinkController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $links = $em->getRepository('AcmeAppBundle:Link')->findAll();
    ...
    {
        $link = new Link();
        $form = $this->createForm('Acme\AppBundle\Form\LinkType', $link, [
            'action' => $this->generateUrl('link_new')
        ]);
	    //'action' => $this->generateUrl('link_new') ne change rien, c'est la page déja affichée
    ...
    {
        $deleteForm = $this->createDeleteForm($link);
        $editForm = $this->createForm('Acme\AppBundle\Form\LinkType', $link);
